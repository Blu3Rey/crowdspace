...
class TransportManager:
    ...
    async def send(self, data: bytes, dst_id: Optional[bytes] = None) -> bool:
        """Transmit *data* bytes on all available channels.

        If *dst_id* is not None and a direct connection to that peer exists,
        we write directly to its RX char *and* notify via the peripheral.
        For broadcast / unknown destinations we send on both channels.

        Returns True if at least one send succeeded.
        """
        success = False

        # ── Central channel: write to known peer(s) ───────────────────────────
        if dst_id is not None:
            ...
            # Broadcast: write to all connected peers
            n = await self.central.send_to_all(data)
            success = success or (n > 0)

        # ── Peripheral channel: notify subscribed centrals ────────────────────
        if self.peripheral:
