...
class DeviceLocator(Feature):
    ...
    async def _handle_req(self, packet: Packet) -> None:
        ...
        resp_payload = struct.pack(_RESP_FMT, packet.src_id, rssi, 1)
    ...
    async def _handle_resp(self, packet: Packet) -> None:
        ...
        # Three possible keys to check:
        #   requester_id — the node that sent the LOC_REQ (= our own node_id for
        #                  responses to our own requests; unused as a pending key
        #                  today but kept for symmetry / future use)
        #   packet.src_id — the responding node's ID; for a targeted locate of
        #                   node X, req_id == X == packet.src_id, so this is the
        #                   key that actually matches _pending in the targeted case
        #   BROADCAST_ADDR — matches the broadcast-locate pending key
