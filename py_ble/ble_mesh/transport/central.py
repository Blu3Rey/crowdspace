...
class BLECentral:
    ...
    async def send_to_all(self, data: bytes) -> int:
        """Broadcast *data* to every connected peer in parallel.  Returns success count."""
        addrs = list(self._clients.keys())
        if not addrs:
            return 0
        results = await asyncio.gather(
            *[self.send_to(addr, data) for addr in addrs],
    ...
    async def _scan_once(self, duration: float) -> None:
        ...
        Passive scanning (``scanning_mode="passive"``) halves radio-on power
        draw compared to active scanning at the cost of not sending scan-request
        packets.  Enable it on battery-constrained nodes via MeshConfig.
        """
        try:
