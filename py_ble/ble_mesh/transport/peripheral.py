...
class BLEPeripheral:
    ...
    def _on_write(
        ...
        # Attempt to extract the actual central's BLE address.
        # bless passes platform-specific identifiers in kwargs:
        #   Linux/BlueZ  — "sender"   : str (MAC address)
        #   macOS CB     — "central"  : CBCentral (has .identifier UUID attribute)
        # Fall back to a sentinel only when neither is present.
        peer_addr: str = "peripheral-rx"
        sender = kwargs.get("sender") or kwargs.get("central")
        if sender is not None:
            if isinstance(sender, str):
                peer_addr = sender
            elif hasattr(sender, "identifier"):
                peer_addr = str(sender.identifier)
        ...
        if self._loop.is_running():
            asyncio.run_coroutine_threadsafe(
                self._rx_handler(raw, peer_addr),
                self._loop,
            )
