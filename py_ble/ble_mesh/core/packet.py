...
#   orig_msg_type: uint8  — MsgType of the reassembled packet (restored on delivery)
_FRAG_FMT = "!HBBB"   # 5 bytes
...
def fragment(
    ...
    return [
        Packet(
            ...
            payload=struct.pack(_FRAG_FMT, frag_id, len(chunks), idx, msg_type) + chunk,
            ttl=ttl,
...
class FragmentAssembler:
    ...
    ``feed()`` returns ``(orig_msg_type, full_payload)`` when all fragments have
    arrived, or ``None`` if the message is still incomplete.  The
    ``orig_msg_type`` is the :class:`MsgType` value carried in the fragment
    sub-header, so the caller can dispatch the reassembled packet to the
    correct feature handler rather than always defaulting to DIRECT_MSG.
    ...
    def __init__(self, timeout: float = 30.0):
        ...
        # key = (src_id, frag_id) → (total_frags, orig_msg_type, created_at)
        self._meta:   Dict[tuple, Tuple[int, int, float]] = {}

    def feed(self, packet: Packet) -> Optional[Tuple[int, bytes]]:
        """Feed one FRAGMENT packet.

        Returns
        -------
        tuple[int, bytes]
            ``(orig_msg_type, reassembled_payload)`` once complete.
        None
            While fragments are still outstanding.
        """
        if len(packet.payload) < FRAG_HEADER_SIZE:
            return None
        frag_id, total, index, orig_msg_type = struct.unpack(
            _FRAG_FMT, packet.payload[:FRAG_HEADER_SIZE]
        )
        ...
        if key not in self._chunks:
            self._chunks[key] = {}
            self._meta[key] = (total, orig_msg_type, time.monotonic())

        self._chunks[key][index] = chunk

        total_stored, stored_type, _ = self._meta[key]
        if len(self._chunks[key]) == total_stored:
            full = b"".join(self._chunks[key][i] for i in range(total_stored))
            del self._chunks[key]
            del self._meta[key]
            return stored_type, full
        return None

    def prune_stale(self) -> int:
        """Remove sessions older than *timeout*.  Returns count removed."""
        now = time.monotonic()
        stale = [k for k, (_, _, t) in self._meta.items() if now - t > self._timeout]
        for k in stale:
