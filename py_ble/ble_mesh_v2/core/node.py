...
class MeshNode:
    ...
    def __init__(self, config: MeshConfig) -> None:
        ...
        if config.enable_encryption and config.psk:
            try:
                self._crypto = MeshCrypto(config.psk, node_id=config.node_id)
    ...
    async def _handle_raw(self, raw: bytes, peer_addr: str) -> None:
        ...
        if packet.msg_type == MsgType.FRAGMENT:
            result = self._assembler.feed(packet)
            if result is None:
                return
            orig_msg_type, reassembled = result
            try:
                packet = Packet(
                    msg_type=orig_msg_type,   # restored from the fragment sub-header
    ...
    async def _forward(self, packet: Packet) -> None:
        """Forward *packet* with TTL decremented, after a short random jitter.

        The jitter (5–50 ms) staggers simultaneous re-broadcasts from multiple
        nodes that all heard the same packet.  Without it, dense meshes suffer
        correlated collisions that multiply packet loss.
        """
    ...
    async def _heartbeat_loop(self) -> None:
        ...
        TTL is capped at 1: heartbeats are purely a neighbour-awareness
        mechanism and have no value beyond one hop.  Forwarding them at the
        default TTL of 7 would flood the entire mesh with O(N × TTL)
        transmissions per cycle for no benefit.
        """
        await asyncio.sleep(1)   # let the peripheral start first
        while self._running:
            payload = self.node_id + self.node_name.encode("utf-8")
            try:
                await self.send(MsgType.HEARTBEAT, payload,
                                dst_id=BROADCAST_ADDR, ttl=1)
                log.debug("[Node] HEARTBEAT broadcast (ttl=1).")
            except Exception as exc:
