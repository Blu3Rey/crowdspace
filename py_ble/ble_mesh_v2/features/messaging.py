...
class DirectMessaging(Feature):
    ...
    def __init__(self, ack_timeout: float = 5.0, max_retries: int = 3) -> None:
        ...
        # Start at a random offset so IDs don't repeat if the node restarts
        # quickly and a peer still has a stale pending-ACK entry for a low ID.
        self._msg_counter = secrets.randbelow(0xFFFFFFFF)
